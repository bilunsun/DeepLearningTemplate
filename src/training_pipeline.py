...
def train(config: DictConfig) -> None:
    ...
    lit_model: pl.LightningModule = hydra.utils.instantiate(config.lit_model, _recursive_=False)
    ...
    logger = hydra.utils.instantiate(config.get("logger"))

    if logger and config.lit_model.get("scheduler_config"):
    ...
    trainer.fit(lit_model, datamodule=datamodule, ckpt_path=config.ckpt_path)
